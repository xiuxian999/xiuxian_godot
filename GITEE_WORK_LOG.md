...
**要求：**
- **导出功能**：一键导出所有概率事件配置为标准 Excel 文件，包含事件 ID、名称、基础概率、保底等级、备注
- **导入功能**：选择 Excel 文件导入配置，自动校验格式，错误提示具体行号
- **实时生效**：导入配置实时生效，无需重启
...
**要求：**
- **三个档位**：关闭所有提示 / 仅核心操作提示 / 全量提示
- **永久保存**：设置永久保存到本地，重启游戏不丢失
- **实时生效**：切换档位实时生效
